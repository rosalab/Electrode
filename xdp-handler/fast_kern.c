...
struct paxos_ctr_state {
	...
	int myIdx, leaderIdx,
		batchSize; // it's easier to maintain in user-space.
	__u64 view, lastOp;
};
...
static inline int compute_message_type(char *payload, void *data_end)
{
	if (payload + PREPARE_TYPE_LEN < data_end && payload[10] == 'v' &&
		...
		return FAST_PROG_XDP_HANDLE_PREPARE;
		   ...
		   payload[10] == 'v' && payload[11] == 'r' &&
		   payload[19] == 'R' && payload[20] == 'e' &&
		   payload[21] == 'q' && payload[22] == 'u' &&
		   payload[23] == 'e' && payload[24] == 's' &&
		   payload[25] == 't' && payload[26] == 'M') {
		// Request message in `vr`.
		return FAST_PROG_XDP_HANDLE_REQUEST;
	} else if (payload + PREPAREOK_TYPE_LEN < data_end &&
		   payload[10] == 'v' && payload[11] == 'r' &&
		   payload[19] == 'P' && payload[20] == 'r' &&
		   payload[21] == 'e' && payload[22] == 'p' &&
		   payload[23] == 'a' && payload[24] == 'r' &&
		   payload[25] == 'e' && payload[26] == 'O') {
		// PrepareOK message in `vr`.
		return FAST_PROG_XDP_HANDLE_PREPAREOK;
	} else if (payload + MYPREPAREOK_TYPE_LEN < data_end &&
		   payload[10] == 'v' && payload[11] == 'r' &&
		   payload[13] == 'M' && payload[14] == 'y' &&
...
int fastPaxos_main(struct xdp_md *ctx)
{
	...
	if (payload + PREPARE_TYPE_LEN < data_end && payload[10] == 'v' &&
	    ...
	    payload[24] == 'r' && payload[25] == 'e' && payload[26] == 'M') {
		// PrepareMessage in `vr`.
		bpf_tail_call(ctx, &map_progs_xdp,
			      FAST_PROG_XDP_HANDLE_PREPARE);
		return XDP_PASS;
	}
#endif
...
int HandleRequest_main(struct xdp_md *ctx)
{
	...
	__u32 zero = 0;
	struct paxos_batch *context =
		bpf_map_lookup_elem(&batch_context, &zero);
	...
	if (num == 0)
		return XDP_PASS; // reach batchSize, notice user-space.
	if (payload + REQ_MAX_DATA_LEN < data_end)
		return XDP_PASS;

	char *pt = bpf_ringbuf_reserve(
		&map_request_buffer,
		REQ_MAX_DATA_LEN + sizeof(__u16) + sizeof(__u32), 0);
	if (pt) {
		*(__u16 *)pt = udp->source;
		pt += sizeof(__u16);
		*(__u32 *)pt = ip->saddr;
		...
		for (int i = 0;
		     i < REQ_MAX_DATA_LEN && payload + i + 1 <= data_end; ++i)
			pt[i] = payload[i];
...
int HandlePrepareOK_main(struct xdp_md *ctx)
{
	...
	if (data + FAST_PAXOS_DATA_LEN > data_end)
		return XDP_DROP;
...
int HandlePrepare_main(struct xdp_md *ctx)
{
	...
	if (payload + MAGIC_LEN + sizeof(__u64) + PREPARE_TYPE_LEN +
	    ...
	    data_end)
		return XDP_PASS;
	payload += MAGIC_LEN + sizeof(__u64) +
		   PREPARE_TYPE_LEN; // point to our extra_data.
	...
	if (msg_batchStart > ctr_state->lastOp + 1)
		return XDP_PASS;
...
int WriteBuffer_main(struct xdp_md *ctx)
{
	...
	if (payload >= data_end)
		return XDP_PASS;
	if (payload + MAX_DATA_LEN < data_end)
		return XDP_PASS;
...
int PrepareFastReply_main(struct xdp_md *ctx)
{
	...
	*(__u64 *)payload = MYPREPAREOK_TYPE_LEN;
	...
	payload[13] = 'M', payload[14] = 'y', payload[15] = 'P',
	payload[16] = 'r', payload[17] = 'e', payload[18] = 'p',
	payload[19] = 'a', payload[20] = 'r', payload[21] = 'e',
	payload[22] = 'O', payload[23] = 'K';
...
int FastBroadCast_main(struct __sk_buff *skb)
{
	...
	if (msg_type == FAST_PROG_XDP_HANDLE_PREPARE) { // clear bitset entry.
		...
		struct paxos_quorum *entry =
			bpf_map_lookup_elem(&map_quorum, &idx);
		if (entry) {
			if (entry->view != msg_view ||
