...
void create_object()
{
	...
	for (int i = 0; i < prog_count; i++) {
		printf("progname: %s\n", progs[i].name);
		progs[i].prog =
			bpf_object__find_program_by_name(obj, progs[i].name);
		if (!progs[i].prog) {
			fprintf(stderr,
	...
	map_progs_xdp_fd =
		bpf_object__find_map_fd_by_name(obj, "map_progs_xdp");
...
void initial_prog_map()
{
	for (int i = 0; i < prog_count; i++) {
		...
		if (prog_fd < 0) {
			fprintf(stderr,
				"Error: Couldn't get file descriptor for program %s\n",
				progs[i].name);
			exit(1); // return 1;
		}
		...
		if (progs[i].map_prog_idx != -1) {
			...
			if (map_prog_idx < 0) {
				fprintf(stderr,
					"Error: Cannot get prog fd for bpf program %s\n",
					progs[i].name);
				exit(1); // return 1;
			}
			...
			default:
				fprintf(stderr,
					"Error: Program type doesn't correspond to any prog array map\n");
				exit(1); // return 1;
			}

			// update map in bpf_tail_call, e.g. f[idx] = fd.
			err = bpf_map_update_elem(map_progs_fd, &map_prog_idx,
						  &prog_fd, 0);
			if (err) {
				fprintf(stderr,
					"Error: bpf_map_update_elem failed for prog array map\n");
				exit(1); // return 1;
			}
		}

		if (progs[i].pin) {
			int len = snprintf(filename, PATH_MAX, "%s/%s",
					   BPF_SYSFS_ROOT, progs[i].name);
			if (len < 0) {
				fprintf(stderr,
					"Error: Program name '%s' is invalid\n",
					progs[i].name);
				exit(-1); // return -1;
			} else if (len >= PATH_MAX) {
				fprintf(stderr,
					"Error: Program name '%s' is too long\n",
					progs[i].name);
				exit(-1); // return -1;
			}
retry:
			if (bpf_program__pin(progs[i].prog, filename)) {
				fprintf(stderr,
					"Error: Failed to pin program '%s' to path %s\n",
					progs[i].name, filename);
				if (errno == EEXIST) {
					fprintf(stdout,
						"BPF program '%s' already pinned, unpinning it to reload it\n",
						progs[i].name);
					if (bpf_program__unpin(progs[i].prog,
							       filename)) {
						fprintf(stderr,
							"Error: Fail to unpin program '%s' at %s\n",
							progs[i].name,
							filename);
						exit(-1);
						// return -1;
...
void read_config()
{
	...
	for (int i = 0; i < 2 * f + 1; ++i) {
		...
		sscanf(eths[i], "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", conf.eth,
...
// gcc -g -O2 -Wall -DKBUILD_MODNAME="\"wzz\"" -I. -I./linux/tools/lib
// -I./linux/tools/include/uapi  -o test fast_test.c
// ./linux/tools/lib/bpf/libbpf.a -L./linux/tools/lib/bpf -l:libbpf.a -lelf  -lz
